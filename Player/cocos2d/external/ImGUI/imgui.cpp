...
#ifdef USE_AGTK//sakihama-h, 2018.06.19, ACT2-2042
int ImFormatStringLength(const char *fmt, va_list args)
{
	return vsnprintf(NULL, 0, fmt, args);
}

char *ImFormatStringMalloc(unsigned int size)
{
	ImGuiState& g = *GImGui;
	if (g.TempBufferSize < size) {
		if (g.TempBuffer) {
			free(g.TempBuffer);
			g.TempBuffer = NULL;
		}
		g.TempBuffer = (char *)malloc(size);
		g.TempBufferSize = size;
	}
	return g.TempBuffer;
}
#endif
...
#ifdef USE_AGTK//sakihama-h, 2018.07.06
void ImGui::ForcedClosePopup()
{
	const char *str_id = "tempName";
	ImGui::OpenPopup(str_id);
	bool bOpendFlag = false;
	if (ImGui::BeginPopupModal(str_id, &bOpendFlag)) {
...
void ImGui::TextV(const char* fmt, va_list args)
{
    ...
    ImGuiState& g = *GImGui;
#ifdef USE_AGTK//sakihama-h, 2018.06.18, ACT2-2042
	ImFormatStringMalloc(ImFormatStringLength(fmt, args) + 1);// malloc
	const char* text_end = g.TempBuffer + ImFormatStringV(g.TempBuffer, g.TempBufferSize, fmt, args);
#else
...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args)
{
    ...
    // Render
#ifdef USE_AGTK//sakihama-h, 2018.06.18, ACT2-2042
	const char* value_text_begin = ImFormatStringMalloc(ImFormatStringLength(fmt, args) + 1);// malloc
	const char* value_text_end = value_text_begin + ImFormatStringV(g.TempBuffer, g.TempBufferSize, fmt, args);
#else
...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
    ...
    const ImGuiStyle& style = g.Style;
#ifdef USE_AGTK//sakihama-h, 2018.06.18, ACT2-2042
	const char* text_begin = ImFormatStringMalloc(ImFormatStringLength(fmt, args) + 1);// malloc
	const char* text_end = text_begin + ImFormatStringV(g.TempBuffer, g.TempBufferSize, fmt, args);
#else
...
#ifdef USE_AGTK//sakihama-h, 2018.06.18, ACT2-2042
	ImFormatStringMalloc(ImFormatStringLength(fmt, args) + 1);// malloc
	ImFormatStringV(g.TempBuffer, g.TempBufferSize, fmt, args);
#else
...
static inline void DataTypeFormatString(ImGuiDataType data_type, void* data_ptr, int decimal_precision, char* buf, int buf_size)
{
	...
	{
		if (decimal_precision < 0)
			ImFormatString(buf, buf_size, "%lf", *(double*)data_ptr);
		else
			ImFormatString(buf, buf_size, "%.*lf", decimal_precision, *(double*)data_ptr);
	}
#endif
}
...
static void DataTypeApplyOpFromText(const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* data_ptr, const char* scalar_format)
{
	...
	else if (data_type == ImGuiDataType_Double)
	{
		...
		else if (op == '/') { if (op_v != 0.0) *v = ref_v / op_v; }    // Divide
		else { *v = op_v; }                              // Assign constant
	}
#endif
}
